...
int main()
{
    ...
    std::cout << std::endl;
    std::cout << "Creating/Opening HeapFile..." << std::endl;
    ...
    std::cout << "Acquiring Slottedpage->.." << std::endl;
    ...
    std::cout << "Content: " << std::unique_ptr<char>((char*)page->get(id)->get_data()).get() << std::endl;
    std::cout << std::endl;
    ...
    for (RecordID &i : *curIds) {
        ...
        try {
            page->get(i);
            std::cout << "Record " << i << " exists with content " << std::unique_ptr<char>((char*)page->get(i)->get_data()).get() << std::endl;
            if (max < i) max = i;
        } catch (...) {
    ...
    for (RecordID i = 1; i <= max; i++) {
        ...
        std::cout << "Content: " << std::unique_ptr<char>((char*)page->get(i)->get_data()).get() << std::endl;
    }

    if (test_heap_storage2()) std::cout << "Prof. Lundeen's tests passed." << std::endl;
    else std::cout << "Prof. Lundeen's tests FAILED!!!!" << std::endl;
